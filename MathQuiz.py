...
#Variable to wait until a certain action has happened
var = IntVar()
...
def generate_question():
    ...
    #slider.get is the difficulty value set by the difficulty slider
    num1 = random.randint(1, slider.get()*50)
    num2 = random.randint(1, slider.get()*50)
    ...
    if operatorchoice == '**':
        if slider.get() < 3:
            num1 = random.randint(1,slider.get()*2+1)
        else:
            #Ensures the value is an integer. If the number is even, subtract 0, if odd, subtract 1.
            num1 = random.randint(1, int(((slider.get()**2)-(slider.get()**2)%2)/2))
        num2 = random.randint(0, slider.get()+1)
    ...
    #Curves the difficulty if the question is square rooted
        ...
        num1 = random.randint(1,slider.get()*5)
        num1 **= 2
...
def math_quiz():
    questions = [generate_question() for i in range(length.get())]
